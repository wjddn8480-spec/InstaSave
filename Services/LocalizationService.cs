...
public static partial class LocalizationService
{
    ...
    public static bool IsKorean => _languageMode switch
    {
        "ko" => true,
        "en" => false,
    ...
    public static string LanguageCode => IsKorean ? "ko" : "en";

    public static void SetLanguageMode(string? mode)
    {
        _languageMode = mode?.Trim().ToLowerInvariant() switch
        {
            "ko" => "ko",
            "en" => "en",
            _ => "auto"
    ...
    {
        ["Instagram 사진·영상 다운로드 · v1.0.0"] = "Instagram photo & video downloader · v1.0.0",
        ...
        ["여러 URL을 줄바꿈으로 입력할 수 있습니다. Ctrl+Enter로 목록에 추가합니다."] = "Enter multiple URLs on separate lines. Press Ctrl+Enter to add them to the queue.",
        ["붙여넣기"] = "Paste",
        ["미리보기"] = "Preview",
        ["목록에 추가"] = "Add to queue",
        ["릴스, 사진 게시물, 동영상 게시물, 여러 미디어가 포함된 게시물 URL을 한 줄에 하나씩 입력하세요."] = "Enter one URL per line for reels, photo posts, video posts, or multi-media posts.",
    ...
    public static string Translate(string? text)
    {
        ...
        return RestoreProtectedValues(result, protectedValues);
    }
