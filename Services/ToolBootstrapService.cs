...
public sealed class ToolBootstrapService
{
    ...
    private const string GalleryDlPinnedVersion = "1.32.6";
    private const string GalleryDlPinnedReleaseApi = "https://codeberg.org/api/v1/repos/mikf/gallery-dl/releases/tags/v1.32.6";
    private const string GalleryDlPinnedDownloadFallback = "https://codeberg.org/mikf/gallery-dl/releases/download/v1.32.6/gallery-dl.exe";
    ...
    public async Task EnsurePhotoEngineInstalledAsync(IProgress<double>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        if (IsValidEngine(AppPaths.GalleryDlPath))
        {
            try
            {
                var installedVersion = await GetPhotoEngineVersionAsync(cancellationToken);
                if (IsPinnedGalleryDlVersion(installedVersion))
                    return;
            ...
            {
                // 버전을 확인할 수 없으면 지정 버전을 다시 설치합니다.
            }
        }

        var downloadUrl = await ResolvePinnedGalleryDlDownloadUrlAsync(cancellationToken);
        await DownloadExecutableAsync(
            downloadUrl,
            AppPaths.GalleryDlPath,
            progress,
            cancellationToken);
    }
    ...
    {
        await ReinstallPhotoEngineAsync(cancellationToken: cancellationToken);
        var installedVersion = await GetPhotoEngineVersionAsync(cancellationToken);
        return $"gallery-dl {installedVersion} 설치 완료 (고정 버전: {GalleryDlPinnedVersion})";
    }
    ...
    private static bool IsPinnedGalleryDlVersion(string versionOutput)
    {
        var normalized = versionOutput.Trim().TrimStart('v', 'V');
        return string.Equals(normalized, GalleryDlPinnedVersion, StringComparison.OrdinalIgnoreCase);
    }

    private async Task<string> ResolvePinnedGalleryDlDownloadUrlAsync(CancellationToken cancellationToken)
    {
        try
        {
            using var response = await _httpClient.GetAsync(GalleryDlPinnedReleaseApi, cancellationToken);
                ...
                assets.ValueKind == JsonValueKind.Array)
            {
                foreach (var asset in assets.EnumerateArray())
                {
                    ...
                    if (asset.TryGetProperty("url", out var assetUrl) &&
                        !string.IsNullOrWhiteSpace(assetUrl.GetString()))
                    {
                        return assetUrl.GetString()!;
                    }
                }
            }
        }
        ...
        {
            // API 조회가 실패하면 고정된 공식 릴리스 주소를 사용합니다.
        }
        catch (JsonException)
        {
            // API 응답을 해석하지 못하면 고정된 공식 릴리스 주소를 사용합니다.
        }

        return GalleryDlPinnedDownloadFallback;
    }
        ...
        CancellationToken cancellationToken)
    {
        var tempPath = Path.Combine(
            Path.GetDirectoryName(destinationPath) ?? AppPaths.ToolsDirectory,
